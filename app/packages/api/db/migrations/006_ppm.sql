-- FMIQ migration 006 — Planned Preventive Maintenance (PPM) scheduling.
-- Target: PostgreSQL 16 (production). Pairs with 006_ppm.dev.sql.
-- See docs/FMIQ-master-build-plan.md §4.1 and docs/FMIQ-operations-modules-spec.md §1.
BEGIN;

-- Tenant-agnostic reference library (SFG20-aligned). No RLS — shared across tenants.
CREATE TABLE wo_task_template (
  ...
  discipline           text,
  required_skill       text,
  standard_ref         text,                 -- SFG20 / CIBSE / Irish SI
  est_minutes          int,
  ...
  default_frequency    text                  -- Postgres interval expression, e.g. '3 mons'
);
...
-- Tenant PPM schedule: one per asset+task.
CREATE TABLE wo_ppm_schedule (
  ...
  frequency        text,                     -- interval expression
...
-- Meter / runtime readings driving condition-based triggers (hypertable).
...
SELECT create_hypertable('wo_meter_reading','ts', if_not_exists => TRUE);
