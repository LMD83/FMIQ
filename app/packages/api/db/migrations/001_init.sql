-- FMIQ schema 001 — tenancy, RLS, estate, collection-care, maintenance, compliance
-- Target: PostgreSQL 16 + TimescaleDB + PostGIS. See ../../../../docs/data-model.md
BEGIN;
...
CREATE TABLE core_tenant (
  ...
  data_region     text NOT NULL DEFAULT 'northeurope',
...
CREATE TABLE core_role (
  id   uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  code text UNIQUE NOT NULL,  -- SystemAdmin, TenantAdmin, FacilitiesManager, ConservationOfficer, MaintenanceTech, ReadOnly
  name text NOT NULL
);
INSERT INTO core_role (code, name) VALUES
  ...
  ('ConservationOfficer','Conservation Officer'),
...
CREATE TABLE core_user_role (
  ...
  role_id   uuid NOT NULL REFERENCES core_role(id),
  PRIMARY KEY (user_id, role_id)
);

-- Append-only audit log (FOI / GDPR accountability). No UPDATE/DELETE grant in app role.
...
CREATE TABLE est_building (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id           uuid NOT NULL REFERENCES core_tenant(id),
  site_id             uuid NOT NULL REFERENCES est_site(id),
  name                text NOT NULL,
  year_built          int,
  protected_structure boolean NOT NULL DEFAULT false,
  national_monument   boolean NOT NULL DEFAULT false,
  gia_m2              numeric,
...
CREATE TABLE est_floor (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id   uuid NOT NULL REFERENCES core_tenant(id),
  building_id uuid NOT NULL REFERENCES est_building(id),
  name        text NOT NULL,
...
CREATE TABLE est_space (
  id                 uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id          uuid NOT NULL REFERENCES core_tenant(id),
  floor_id           uuid NOT NULL REFERENCES est_floor(id),
  name               text NOT NULL,
  space_type         text NOT NULL CHECK (space_type IN ('gallery','store','office','plant','public','circulation')),
  nia_m2             numeric,
  cost_centre        text,
  is_collection_zone boolean NOT NULL DEFAULT false
);
...
CREATE TABLE est_asset (
  ...
  criticality    text,
  qr_uid         text,
  parent_asset_id uuid REFERENCES est_asset(id)
);
...
CREATE TABLE cc_case (
  id        uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES core_tenant(id),
  space_id  uuid NOT NULL REFERENCES est_space(id),
  name      text NOT NULL,
  case_type text CHECK (case_type IN ('display','vitrine','drawer','shelf')),
...
CREATE TABLE cc_zone (
  ...
  tenant_id uuid NOT NULL REFERENCES core_tenant(id),
  space_id  uuid NOT NULL REFERENCES est_space(id),
  cc_case_id uuid REFERENCES cc_case(id),
  name      text NOT NULL
);

CREATE TABLE cc_standard (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  code        text UNIQUE NOT NULL,  -- PAS198 | BS4971 | ASHRAE_AA..D | BIZOT_2023 | CUSTOM
  name        text NOT NULL,
  description text
);
...
CREATE TABLE cc_zone_target (
  ...
  cc_standard_id     uuid NOT NULL REFERENCES cc_standard(id),
  active             boolean NOT NULL DEFAULT true,
  temp_min           numeric, temp_max numeric,
  rh_min             numeric, rh_max numeric,
  rh_rate_max_per_24h numeric,   -- max permitted RH swing per 24h (rate-of-change)
  lux_max            numeric,
  uv_max_uw_per_lm   numeric,
  co2_max_ppm        numeric
);
...
-- High-frequency readings → TimescaleDB hypertable
CREATE TABLE cc_reading (
  tenant_id uuid NOT NULL,
  sensor_id uuid NOT NULL,
  zone_id   uuid NOT NULL,
  metric    text NOT NULL,  -- temp | rh | lux | uv | co2 | voc | shock
  value     double precision NOT NULL,
  unit      text,
  quality   text DEFAULT 'good',
  ts        timestamptz NOT NULL DEFAULT now()
);
SELECT create_hypertable('cc_reading','ts', if_not_exists => TRUE);
CREATE INDEX ix_cc_reading_zone_metric ON cc_reading (tenant_id, zone_id, metric, ts DESC);

-- Hourly continuous aggregate for dashboards
CREATE MATERIALIZED VIEW cc_reading_hourly
WITH (timescaledb.continuous) AS
  SELECT tenant_id, zone_id, metric,
         time_bucket('1 hour', ts) AS bucket,
         avg(value) AS avg, min(value) AS min, max(value) AS max
  FROM cc_reading
  GROUP BY tenant_id, zone_id, metric, bucket
WITH NO DATA;
...
CREATE TABLE cc_excursion (
  ...
  tenant_id        uuid NOT NULL REFERENCES core_tenant(id),
  cc_zone_id       uuid NOT NULL REFERENCES cc_zone(id),
  cc_zone_target_id uuid REFERENCES cc_zone_target(id),
  metric           text NOT NULL,
  kind             text NOT NULL CHECK (kind IN ('absolute','rate_of_change')),
...
CREATE TABLE cc_object_link (
  ...
  tenant_id  uuid NOT NULL REFERENCES core_tenant(id),
  cc_zone_id uuid NOT NULL REFERENCES cc_zone(id),
  cms_vendor text CHECK (cms_vendor IN ('axiell','tms','mimsy')),
  cms_object_id text NOT NULL,
  object_name text NOT NULL,
  material    text,
  sensitivity text NOT NULL DEFAULT 'med' CHECK (sensitivity IN ('low','med','high')),
...
-- =====================================================================
-- Row-Level Security — apply to every tenant-scoped table
-- =====================================================================
DO $$
DECLARE t text;
BEGIN
...
-- Dedicated, least-privilege application role (never a superuser / BYPASSRLS for routine queries)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fmiq_app') THEN
    CREATE ROLE fmiq_app LOGIN PASSWORD 'fmiq_app';
  END IF;
END $$;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO fmiq_app;
GRANT USAGE ON SCHEMA public TO fmiq_app;
-- audit log is append-only for the app role
REVOKE UPDATE, DELETE ON core_audit_log FROM fmiq_app;
